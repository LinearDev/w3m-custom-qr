...
let W3mBuyInProgressView = class W3mBuyInProgressView extends LitElement {
    ...
    async fetchCoinbaseTransactions() {
        ...
        const pendingTransactions = coinbaseResponse.data.filter(tx => tx.metadata.status === 'ONRAMP_TRANSACTION_STATUS_IN_PROGRESS');
        if (pendingTransactions.length && this.intervalId) {
            clearInterval(this.intervalId);
            RouterController.replace('OnRampActivity');
        }
        else if (this.startTime && Date.now() - this.startTime >= 180000 && this.intervalId) {
            clearInterval(this.intervalId);
            this.error = true;
        }
    }
